...
%	\end{figure}
	...
	Ewunetu（2025）使用谷歌地球引擎（GEE）和随机森林（RF）分类器，结合多年的 Landsat 遥感数据和当地居民与长老的实地数据，分析了上特克兹盆地的土地利用/土地覆盖变化及其原因\cite{ewunetu_integrating_2025}。此外，Eisavi, V.等（2015）使用Landsat 8多时相光谱和热成像数据的潜力，采用随机森林分类器对伊朗西阿塞拜疆省Naghadeh市及其周边地区进行土地覆盖分类，结果表明，选取重要特征和多时相热信息能显著提高分类精度\cite{eisavi_land_2015}。
