...
\begin{document}
	...
	\par 三、集成学习模型比较与预测
	\par 采用多种集成学习方法（如随机森林、梯度提升机等）进行植物病害的预测分析，比较了多种模型的优劣，并评估各变量的重要性。基于随机森林的预测结果，我们绘制了中国地区植物病害的大尺度预测图，并进一步模拟了气候变化条件下的植物病害分布。
	\par 四、深度学习建模与优化
	\par 采用PyTorch和TensorFlow分别构建神经网络模型，并进行训练和优化。比较了卷积神经网络和全连接神经网络等深度学习模型的预测精度。
	...
	\EnAbstract{
		...
		\par Various ensemble learning methods (such as random forests, gradient boosting machines, etc.) were employed for plant disease prediction analysis. The strengths and weaknesses of multiple models were compared, and the importance of each variable was assessed. Based on the prediction results of the random forest model, a large-scale prediction map of plant diseases across China was created, and further simulations of plant disease distribution under climate change scenarios were conducted.
		
		\par 4. Deep Learning Model Development and Optimization
		\par Neural network models were constructed using PyTorch and TensorFlow, followed by training and optimization. The prediction accuracy of convolutional neural networks and fully connected neural networks, among other deep learning models, was compared.
	...
	农业是对人类生活和经济状况影响较大的行业之一。农民缺乏对病害的认知，导致粮食产量下降\cite{9395751}。
	...
	到了文艺复兴后期，科学革命带来了观察和实验的兴起，列文虎克 (Antonie van Leeuwenhoek) 的显微镜发明揭开了微观世界的面纱。他首次通过显微镜观察到微生物的存在，为人类识认植物病害的病原基础。虽然他并未直接研究植物病害，但他的发现激发了后人从科学角度探索病害成因的兴趣\cite{doi:10.1098/rstl.1677.0003}。
	...
	茭白的独特生长便得益于其与植物病害之间的关系。茭白因感染了一种特定的真菌而形成了膨大的茎，这种病害实际上是茭白特有的品质来源，使其成为一种颇受欢迎的水生蔬菜。
	...
	植物在生长过程中受到多种病原体的侵害，其中病原真菌和卵菌是主要的病原体，严重影响植物的生长和发育。\cite{mendgen_plant_2002}
	锈菌、白粉菌以及卵菌中的疫霉菌和霜霉菌是导致植物病害的主要原因，\cite{annurev:/content/journals/10.1146/annurev.phyto.46.081407.104740}造成植物形态异常、
	功能受损和生理受限，进而引发一系列植物病害 \cite{latijnhouwers_oomycetes_2003}。这些病害不仅影响植物的生长，还对农业生产造成显著威胁，导致农作物减产和品质下降 \cite{https://doi.org/10.1002/ps.565, 19971001256}。
	
	锈病（Rust disease）通常表现为植物叶片和茎秆上出现小斑点，随着病情加重，可能导致叶片脱落和植株枯死。\cite{ https://doi.org/10.1111/mpp.12190}
	该病害的发生与环境湿度、温度及病原菌的传播密切相关，在高湿环境下，锈病病原体更易繁殖，导致病害的迅速扩散\cite{ annurev:/content/journals/10.1146/annurev.py.30.090192.003005}。
	白粉病（Powdery mildew）主要表现为植物表面覆盖一层白色粉状物，严重影响植物的光合作用，进而影响其生长。白粉病的病原菌在温暖、干燥的环境中更容易传播，导致大规模的植物感染 \cite{ yarwood_powdery_1957}。叶斑病（Leaf spot disease）的特征是叶片上出现各种颜色的斑点，随着病情的加重，斑点逐渐扩散，最终导致叶片枯萎 \cite{agrios2005plant, https://doi.org/10.1046/j.1364-3703.2003.00173.x}。
	
	植物病害的分类可以从多个方面进行划分，主要包括病原、受害部位、症状表现、传播途径、病害的生活史类型以及侵染性和非侵染性病害等。按照病原分类，植物病害可以分为真菌、细菌、病毒、线虫等生物性病害，以及由干旱、盐害、缺素等非生物因子引起的病害。稻瘟病属于真菌病害，而烟草花叶病毒病则是典型的病毒病害\cite{annurev:/content/journals/10.1146/annurev.phyto.42.040803.140322, JONES2006205}。根据受害部位，病害可能影响植物的根部、茎部、叶部或果实，甚至是整个植株，像根结线虫病通常集中在根部，而白粉病则主要发生在叶片上\cite{annurev:/content/journals/10.1146/annurev.phyto.46.081407.104740}。若从症状表现分类，植物病害可能表现为枯萎、腐烂、斑点、变色或畸形等现象。
	枯萎病会导致植物整株萎蔫，叶斑病则会在叶片上形成斑点，而黄化病则使植物整体变黄失绿\cite{LinLawellGoodhueCarterCarroll+2019+217+250}。

	从传播途径来看，某些病害通过土壤传播，如根腐病；有些通过空气或昆虫媒介传播，例如小麦锈病和花叶病毒病；还有一些病害通过种子传播，像稻瘟病的病原可以依附在种子表面或潜藏在种子内部，成为下一个生长季病害的来源\cite{B100803J, doi:10.1094/PDIS-08-12-0791-FE}。

	
	植物病害菌的生活史类型也影响其传播方式。
	生活史较简单的病害如稻瘟病，其病原体通过孢子、种子或土壤传播，病原菌能够在季节更替时通过病残体存活，一旦条件适宜，便迅速繁殖并感染寄主 \cite{annurev:/content/journals/10.1146/annurev.phyto.40.030402.110010}。
	生活史较复杂的病害如梨锈病，其病原菌需要在两种寄主之间交替完成生命周期，春季时从桧柏上产生的锈孢子传播至梨树，在梨树叶片或果实上引发病害并形成孢子器，然后这些孢子通过风传播回桧柏，从而完成病原的循环传播。这种寄主交替的特点使得锈病防控变得尤为复杂，需要通过寄主隔离、修剪清理和药剂喷施等多种方式才能有效控制病害的传播 \cite{palmieri2022}。
	...
	Struck(2006)研究了真菌在腐生与寄生生活阶段之间的转换，病原物大体分为两类：一类病原物杀死寄主，然后从上面获得营养物质，即所谓的死体营养寄生物；另一类是需要获得寄主以完成它们的生活史，即活体营养寄生物。死体病害菌一般具有较强的腐蚀性，可以对多种寄主造成侵害，通常可以用木制培养基培养。而活体病害菌的专一性比较强，一般只能寄生于特定的寄主，形成特定的蛋白质机构从寄主细胞上获取营养物质，一般认为不能够脱离寄主存活。
	活体病原菌的一个短暂阶段代表了半活体营养病原菌。这类真菌在开始转向杀死寄主之前具有一个活体营养生长阶段。
	
	
	\subsection{植物病害对于生态环境的影响}
	
	植物病害是影响生态系统功能和稳定性的重要因素，其对生态环境的影响具有多样性和复杂性。
	从个体植物到整个生态系统，病害不仅降低了植物的健康水平，还可能对物种多样性、栖息地结构和生态过程造成深远的影响 \cite{https://doi.org/10.1890/10-2241.1}。
	病原微生物的传播和感染可能改变植物群落的组成和竞争格局，一些致病真菌能够选择性地感染优势种，从而导致生态系统中的物种替代现象 \cite{Garrett2006}。
	这种影响在生物多样性热点地区尤为显著，因为那里的物种密度较高，传播路径更为复杂。
	植物病害对碳循环和养分循环也具有显著影响。
	病害引发的植被减少会降低碳固定能力，削弱生态系统对温室气体的吸收能力，同时病害引发的枯枝落叶分解加速可能增加土壤中碳的释放，进而加剧气候变化的影响 \cite{https://doi.org/10.1111/j.1365-2486.2008.01716.x,}。
	此外，植物病害可能通过改变根际微生物群落和土壤养分平衡，进一步影响植物与土壤之间的反馈关系，形成一种复杂的负向循环 \cite{ruotsalainen_root_2011}。
	在更大的生态系统范围内，植物病害会影响物种间的相互作用，包括授粉者和种子传播者的活动。
	当某种植物由于病害而种群数量锐减时，依赖该植物的动物也会受到连锁反应的影响，进一步改变生态网络的动态平衡 \cite{annurev:/content/journals/10.1146/annurev.energy.30.050504.144212}。
	特别是在入侵物种或引入性病害的场景下，其影响可能更具毁灭性，栗疫病（\textit{Cryphonectria parasitica}）在北美地区导致了栗树几乎完全灭绝，这一事件显著改变了森林生态系统的结构和功能 \cite{anagnostakis1987chestnut}。
	...
	某些病害可能使植物的根系受到损害，导致水分和养分吸收能力下降，从而影响整个生态系统的水分循环和养分循环\cite{Gupta2017}。
	更严重的是，一些病害可以通过影响植物的光合作用过程，降低植物的碳固存能力，从而对全球碳循环产生负面影响\cite{https://doi.org/10.1111/j.1365-2486.2009.02038.x}。此外，病害的爆发可能促进某些害虫种群的繁殖，进一步加剧生态系统的不稳定性。
	总的来说，植物病害对生态环境的影响是多方面的，涉及植物健康、物种间的竞争与合作、生态系统的功能与服务等各个层面。


	植物抵御病害的方式多种多样，其中早熟是其中一种重要的适应机制。通过加速生长周期，早熟的植物能够在病害发生之前完成生长发育，减少病原菌的侵入机会（Velásquez AC，2018）\cite{velasquez_plant-pathogen_2018}。
	一些作物通过选择早熟品种或通过外部环境调节使得作物提前进入生长高峰期，从而在病害高发时节未受到过多的影响。
	另一重要的防御机制是气孔的开闭 \cite{plants12193380}。气孔是植物进行气体交换的主要通道，然而，病原微生物通常通过气孔进入植物体内。
	为了避免病原的侵入，植物能够通过调节气孔的开闭来控制病害的扩散。在面对病原威胁时，植物会关闭气孔，从而减少病原通过气孔进入植物体内的机会 \cite{genes9070339}。
	此外，植物在遇到病害时，常常通过气孔的反应与局部的免疫反应相结合，启动一系列抗病机制 。
	通过这些生理调节，植物能在不同的环境条件下及时做出反应，有效抵御病害 \cite{HU2020127}。气孔的闭合与植物的免疫反应紧密相连，共同作用以提升植物的抗病能力 \cite{peng2022stomata}。
	
	总之，植物的抗病性和感病性是在与病害的长期演化过程中形成的生理和遗传特性，植物通过调整生长周期、气孔的开闭等方式，强化自身的防御机制，减少病害的侵害。
	Deng（2020）通过转录组测序揭示了某些植物病原真菌的致病机制，提供了新的靶点用于抗病性品种的育种\cite{deng_molecular_2020}。Harper Glyn（2002）研究了新型植物病毒的基因组特征，阐明了其在植物中的传播机制，为植物病毒病害的监测和防控提供了理论基础\cite{annurev:/content/journals/10.1146/annurev.phyto.40.120301.105642}。
	Yin, Kangquan探讨了如何通过基因编辑技术提高作物对细菌、真菌和病毒性疾病的抗性，并展望了基因编辑在未来培育具有新型抗病性的作物方面的潜力。\cite{doi:10.1098/rstb.2018.0322}
	植物的免疫机制是植物病害研究的另一个重要领域。研究发现，植物通过感知病原体的特征，激活自身的免疫反应，从而抵御病害的侵袭。
	Mengiste（2012）总结了植物对腐生性病原的免疫反应的最新进展，并与植物对营养性病原的免疫反应进行了比较，强调了它们的共同机制和对比机制。\cite{annurev:/content/journals/10.1146/annurev-phyto-081211-172955}
	在病害管理策略方面，Ons Lena提出了一种将生物防治剂与化学杀菌剂相结合的综合方法，以减少化学杀菌剂的使用量和残留，从而有效管理植物病害，并减缓病原菌抗药性的产生。\cite{microorganisms8121930}此外，智能农业技术的应用也为病害监测与管理提供了新机遇。
	Khattab Ahmed（2019）开发了一种基于物联网的监测系统，应用于精准农业，特别是流行病控制。\cite{KHATTAB2019105028}
	...
	气候变化和全球变暖导致温度的升高和部分地区降水格局的改变，正在加剧这些病害的发生和传播。温暖潮湿的环境有利于病原体的繁殖和扩散，导致病害在更大范围内更频繁地发生。
	全球变暖导致的温度升高和降水模式的改变，正促使一些病原真菌和卵菌向新的地理区域扩展，这些区域以前并不适合它们的生存和繁殖。气候变化影响了植物的生理状态，使其更易受到病害侵染。\cite{pautasso_impacts_2012}实际上，温度和降水是影响叶片真菌病害的主要环境因子 \citep{ Bebber2013, Garrett2006, Velasquez2018, Delgado2020}。
	...
	草地对动物产业、土壤保护和生物多样性至关重要，但植物病害会降低产量和营养价值。\citep{Chakraborty2011}
	病害选择性地影响了某些物种，从而减少了群落内的物种多样性和丰富度\cite{doi:10.1094/PHYTO-99-11-1228}。
	Sarah J. Gurr （2018）使用广义线性模型的研究发现，真菌和昆虫每年向两极迁移约7公里。
	...
	与此同时，二氧化碳浓度的升高导致植物病原体的感染能力增强\cite{gullino_effect_2018}。
	在年均降水增加和年均温度升高的条件下，杂草表现出显著的病害和无脊椎动物损害增加的趋势，尤其是在高温高湿的环境中更为明显。
	相反，草类和豆科植物对这些环境因素的响应相对稳定，没有显示出明显的损害程度增加的趋势\cite{https://doi.org/10.1111/1365-2745.13801}。

	气候变化导致部分地区的极端天气增多，进而导致植物病害严重程度改变\cite{plants13172447}。
	
	\subsection{影响植物病害的土壤因子}
	
	土壤作为植物生长的基础，其化学元素的组成对植物的健康和抵抗病害的能力具有深远影响 \cite{19950708651}。  
	从化学元素的角度，植物需要的大量元素（如氮、磷、钾）和微量元素（如锌、铁、硼）共同作用，决定了植物生长的质量和抗病能力 \cite{Garcia-Mina2012}。  
	土壤中这些元素的供应平衡，不仅影响植物的正常生理代谢，还能增强其对病原微生物的抵抗力 \cite{Tripathi2022}。  
	常量元素，如氮（N）、磷（P）和钾（K），是植物生长的基本需求。  
	氮是叶绿素合成和光合作用的关键，而磷参与能量转移和根系发育，钾则能增强植物的抗逆性，提高细胞壁的稳定性和病原菌侵染后的修复能力 \cite{JIAYING2022166}。  
	在番茄的生长中，适量的钾供应可以提高果实的品质和植物的抗病能力，而氮磷的平衡能够促进番茄生长，同时减少根部病害的发生 \cite{https://doi.org/10.1111/j.1399-3054.2008.01075.x}。  
	在水稻中，钾能够显著增强对稻瘟病的抗性，而磷的充足供应有助于水稻的根系发育，增强其吸收能力和病害抵抗力 \cite{doi:https://doi.org/10.2136/sssabookser4.2ed.c10}。  
	微量元素虽然需求量小，却对植物的健康和病害防治至关重要 \cite{Garcia-Mina2012}。  
	锌（Zn）在植物中参与多种酶的活性调控，能够增强对病原菌的抵抗力；硼（B）对于细胞壁的形成和结构稳定至关重要，缺硼容易导致植物细胞壁薄弱，病害易于侵入 \cite{Tripathi2022}。  
	在番茄的生长中，硼的不足可能导致果实发育不良，增加病害的发生率 \cite{https://doi.org/10.1111/j.1399-3054.2008.01075.x}。  
	而在水稻中，锌的缺乏会导致植株矮小、叶片黄化，从而增加稻瘟病的风险 \cite{JIAYING2022166}。  
	因此，土壤中元素的种类、含量和比例直接影响植物的生长状态和病害抵抗能力 \cite{19950708651}。  
	土壤中的化学元素通过调控植物的生理状态，增强其病害抵抗能力。此外，合理的土壤管理，如补充有机质和调整pH值，还能优化元素的吸收效率 \cite{Tripathi2022}。  
	适当的有机肥能够提供微量元素，同时改善土壤结构，增强植物的根系活力。这些因素共同作用，使得健康的土壤不仅是植物生长的基石，也是植物抵抗病害的重要屏障 \cite{Garcia-Mina2012}。

	\section{影响植物病害的生物因子}
	
	\subsection{植物多样性}
	植物多样性对植物病害的发生具有显著影响。植物多样性可以通过多种机制调节病害的传播和发生，其中最为重要的机制包括生态位分化、竞争作用和共生互作等。
	植物多样性较高的生态系统通常具备更强的抵抗病害的能力，因为在这些系统中，植物种类之间的竞争和生态位的分化会有效地限制病原的扩散。
	具体来说，植物多样性较高的环境可以减少病原在某一植物种群中的集中度，从而减轻病原的传播风险\cite{https://doi.org/10.1890/10-2241.1}。
	此外，多样性还可能通过“群落抗性”效应，即某些植物通过化学或物理屏障抵抗病害，来抑制病原在群落中的传播。
	然而，植物多样性的影响并非总是积极的，某些情况下，高植物多样性反而可能导致病害的增加。
	一些植物种类可能通过提供更为丰富的宿主资源，促进了病原的传播，特别是在植物种类间没有充分的竞争和抑制作用时。这种情况在引入外来病害或病原时尤其明显，其中外来物种可能通过与本地植物的相互作用，增强病害的发生频率\cite{Garrett2006}。
	在农业系统中，作物多样性被认为是一种有效的病害管理策略。
	多样化的农作物种植可以减少单一作物的大规模病害爆发，因为病原体难以在多个作物间传播，特别是当这些作物的病害敏感性各不相同时。
	轮作和多种植系统通过引入不同的作物种类和轮换种植模式，打破了病原的生命周期，从而有效减轻了土传病害的发生。
	此外，植物多样性还与植物之间的相互作用密切相关，如共生微生物群落的变化可能影响植物对病原的抵抗能力。
	某些植物通过与土壤中的有益微生物，如根际真菌和细菌，建立共生关系，从而提高了植物的免疫反应和对病害的防御能力。\cite{song2015enhanced}
	...
	在多样性较高的植物群落中，植物种间的竞争和生态位分化常常能够限制病原的扩散，因为病原难以在多个物种之间传播，尤其是当这些物种具备不同的抗病特性时\cite{https://doi.org/10.1890/10-2241.1}。
	此外，多样化的植物物种组成有助于提升群落整体的抗病能力，某些植物通过化学屏障或物理屏障对病原进行抑制，从而减少病原的感染机会。
	然而，物种组成对病害的影响并非总是积极的。
	在一些情况下，植物群落中物种组成的变化可能反而促进病害的传播。
	在一些由外来植物物种主导的群落中，这些外来物种可能为病原提供了更多的宿主资源，导致病害的传播加剧\cite{Garrett2006}。
	此外，当植物物种组成中的某些植物种类对病原具有较弱的抗性时，它们可能成为病原的“桥头堡”，使病原能够迅速传播至整个群落。\cite{mitchell2006disease}
	在这种情况下，植物物种的均衡与合理配置变得尤为重要，尤其是在农业系统中，作物种类的选择和布局可以有效地减轻病害的发生。
	植物物种的组成还通过影响植物-病原之间的互作关系，间接影响病害的发生。
	植物种类之间的相互作用可以改变植物对病原的免疫反应。
	一些植物可能通过与土壤中的有益微生物形成共生关系，增强其对病原的抵抗力，而某些植物则可能通过改变土壤环境或释放化学物质来干扰病原的扩散\cite{bastias_environmental_2022}。
	在这些复杂的植物-病原-环境相互作用下，物种组成的变化会显著改变病害的发生模式。\cite{PILOSOF2017237}
	...
	在物种均匀度较高的生态系统中，植物种群的健康状况较好，病原的传播受到抑制，
	这可能是因为在高均匀度群落中，病原难以在单一物种上迅速扩散，从而减少了病害的发生频率\cite{https://doi.org/10.1890/10-2241.1}。
	然而，物种均匀度对植物病害的影响并非总是积极的。
	在某些情况下，高均匀度的群落可能会促进病害的传播。特别是当物种间抗病性差异较小，或某些物种对病原的抗性较弱时，病原可能在均匀分布的植物种群中更容易传播。\cite{shaffery_note_2020}
	...
	这种情况下，病原的多样性和丰度可能会随着宿主种类的增加而上升，从而促进病害的发生。
...
%	\end{figure}
    ...
    \begin{tabular}{ lclclc}
		...
		类别 & 计数 & 类别 & 计数 & 类别 & 计数 \\
		\hline
		甘肃省 & 13 & 云南省 & 13 & 河北省 & 12 \\
		陕西省 & 11 & 黑龙江省 & 11 & 贵州省 & 10 \\
		广西壮族自治区 & 10 & 广东省 & 10 & 吉林省 & 9 \\
		山东省 & 9 & 河南省 & 9 & 湖北省 & 9 \\
		湖南省 & 9 & 内蒙古自治区 & 8 & 江苏省 & 8 \\
		安徽省 & 8 & 四川省 & 8 & 山西省 & 7 \\
		江西省 & 7 & 福建省 & 7 & 新疆维吾尔自治区 & 6 \\
		浙江省 & 5 & 辽宁省 & 4 & 重庆市 & 4 \\
		海南省 & 4 & 宁夏回族自治区 & 2 &  &  \\
		\hline
	\end{tabular}
	...
	代码对以特定前缀命名的列（如 \texttt{prec\_} 表示降水量）进行了求和操作，将结果存储为新变量（如 \texttt{MAP} 表示年均降水量）。类似地，对于其他气候变量，如 \texttt{wind\_}、\texttt{tmax\_}、\texttt{tmin\_} 和 \texttt{tavg\_}，分别计算了总和或平均值，并生成了新的变量， \texttt{WIND}（总风速）、\texttt{MAXMAT}（最大平均气温）、\texttt{MINMAT}（最小平均气温）和 \texttt{AVGMAT}（平均气温）。此外，还处理了变量 \texttt{srad\_} 和 \texttt{vapr\_}，分别生成了代表辐射和蒸汽压力总和的变量 \texttt{SRAD} 和 \texttt{VAPR}。生成这些综合指标后，原始的气候相关列被移除，以减少冗余。
	
	完成变量构造后，代码进一步清理了数据，删除了一些不需要的列（如 \texttt{REF\_DEPTH}、\texttt{LANDMASK}、\texttt{ROOTS} 和 \texttt{ISSOIL}），并将剩余列名统一转化为大写格式。最后，将目标变量 \texttt{RATIO} 与其他特征分离，准备用于模型训练和测试。在建模阶段，代码首先将数据分为训练集和测试集（比例为 7:3），并使用随机森林回归模型作为基模型。为了进一步提升特征选择的科学性与稳健性，代码使用了 \texttt{Boruta} 算法对训练集特征进行重要性排序 \cite{kursa2010feature}
	...
	\]
	其中： \( \rho \) 为 Spearman 相关系数；
	 \( d_i \) 为每一对数据点在排序后之差的平方；
	 \( n \) 为样本数量。\text{Spearman 相关系数的 p 值}：\[
	...
	为了分析自变量和因变量之间的关系，我们计算了自变量与因变量之间的距离矩阵，采用欧几里得距离度量。在此基础上，我们定义了一个 mantel\_test 函数，计算了自变量和因变量的距离矩阵之间的 Pearson 相关系数和 p 值，来评估它们之间的空间相关性。通过遍历每个自变量列，我们完成了每个自变量与因变量的 Mantel 检验，并将结果保留为两位小数，同时对 p 值小于 0.05 的结果进行加粗显示，表示其具有显著性。
		我们使用了必要的 R 库，包括用于生态数据分析的 \texttt{vegan} 和 \texttt{dplyr}，以及 \texttt{linkET} 和 \texttt{corrr} 等用于计算相关性和绘图的库。接着，设置了环境变量 \texttt{LD\_LIBRARY\_PATH}，并安装了所需的包。在数据部分，代码指定了工作目录并读取了名为 \texttt{selection.csv} 的数据文件。然后，使用 \texttt{dplyr} 包重命名数据中的一列，将原本名为 \texttt{RATIO} 的列重命名为 \texttt{Pathogen Load}。接下来，提取了除 \texttt{Pathogen Load} 外的所有其他列，并将这些列分配给变量 \texttt{xxxx}，而 \texttt{Pathogen Load} 列被提取到变量 \texttt{yyyy} 中，作为因变量。接下来定义了三组自变量：\texttt{Climate}、\texttt{Soil} 和 \texttt{Geo}，它们分别表示与气候、土壤和地理相关的变量。使用 \texttt{dplyr} 的 \texttt{select} 函数重新排序 \texttt{xxxx} 数据框中的列，以确保它们按照指定的组别排列。
	
	随后，利用 \texttt{mantel\_test} 函数进行了 Mantel 检验，以计算变量之间的相关性，特别是在自变量和因变量之间的关系。通过这个检验，计算得到了相关系数和 p 值，并将它们分类成不同的区间，以便进一步分析。最后，使用 \texttt{qcorrplot} 函数绘制了一个相关系数矩阵图，展示了变量间的相关性，并通过自定义的 \texttt{geom\_couple} 和 \texttt{scale\_*} 函数将 Mantel 检验的结果（包括 p 值和 r 值）以不同的方式呈现，确保图形不仅展示相关系数矩阵，还能展示相关性的显著性和强度。这些图形和检验结果可以帮助更好地理解变量之间的关系，尤其是植物病害与气候、土壤和地理因素的相互作用。
	\text{欧几里得距离}：
	\[
	...
	大多数气候和土壤变量与响应变量之间具有显著的相关性，特别是太阳辐射 (SRAD)、土壤砂含量 (S SAND)、气压 (VAPR)、海拔 (ELEV)、纬度 (LAT) 等与植物病害之间的相关性显示出非常显著的负相关关系。另一方面，风速 (WIND) 和表层土壤砾石含量 (T GRAVEL) 与响应变量之间的相关性并不显著，p 值较高，分别为 0.38 和 0.89。
...
%	\end{figure}
	...
	GDAL（Geospatial Data Abstraction Library）是一个开源的地理空间数据处理库，广泛应用于地理信息系统（GIS）领域，具有强大的数据读写和处理功能 \cite{Warmerdam2008}。
	...
	贝叶斯回归的核心思想是将回归问题转化为对模型参数的概率推断，而不是通过点估计来确定参数值 \cite{gelman1995bayesian}。
	...
	基于核方法的贝叶斯回归（如高斯过程回归）通过使用核函数来处理非线性回归问题。
	高斯过程回归的核心思想是利用高斯过程作为先验，并通过观测数据更新后验，从而实现对非线性函数的建模。
	
	在实际应用中，贝叶斯回归广泛应用于多个领域，包括生物学（Darren J. Wilkinson，2007）\cite{wilkinson_bayesian_2007}、金融学（Zhao Xuejun，2021）\cite{Zhao2021}、工程学以及人工智能等。
	在金融市场中，贝叶斯回归被用于风险预测和资产定价问题（Axel Gandy，2017）\cite{doi:10.1287/mnsc.2016.2546}。
	...
	结构方程模型的基本框架包括两个主要部分：测量模型和结构模型。测量模型用于描述潜在变量与观测变量之间的关系，通常通过因子分析来构建。结构模型则用于描述潜在变量之间的因果关系。潜在变量通常是通过一组与之相关的观测变量来进行测量的，而测量误差会对变量间的关系产生影响，这一点在 SEM 中得到了明确的处理\cite{jenatabadi2015tutorialanalyzingstructuralequation}。在 SEM 中，通常将模型分为两个部分：一是测量模型（Measurement Model），它描述了潜在变量与观测变量之间的关系，二是结构模型（Structural Model），它描述了潜在变量之间的因果关系。测量模型通常使用因子分析来建模，而结构模型则通过回归方程来表示因果关系\cite{byrne2010}。结构方程模型的数学表达式通常涉及到回归方程，潜在变量和观测变量之间的关系可以用以下的公式来表示：
	
	\[
	\mathbf{Y} = \mathbf{B} \mathbf{X} + \mathbf{E}
	\]
	
	其中，\(\mathbf{Y}\) 表示因变量（即观测变量），\(\mathbf{X}\) 表示自变量（即潜在变量或观测变量），\(\mathbf{B}\) 是回归系数，\(\mathbf{E}\) 是误差项。在结构方程模型中，潜在变量和观测变量的关系可以通过路径图（Path Diagram）来进行可视化。路径图中的箭头表示变量之间的因果关系，箭头的方向通常从自变量指向因变量\cite{byrne2010}。SEM 的优势之一是其能够处理潜在变量的测量误差，这使得它比传统的回归分析方法更为精确。此外，结构方程模型也能够处理多个依赖关系和多条因果路径，这是传统线性回归模型无法做到的。然而，SEM 也存在一定的局限性，模型的拟合程度较难评价，且模型的估计结果依赖于数据的质量和假设的合理性\cite{wang2010}。在实际应用中，SEM 的模型拟合通常通过最大似然估计（Maximum Likelihood Estimation，MLE）来实现。拟合优度指标如卡方检验（\(\chi^2\)）、比较拟合指数（CFI）、均方根误差近似（RMSEA）等被用来评估模型的拟合效果。RMSEA值小于0.05表示模型拟合良好，CFI值大于0.90表示模型的拟合较好\cite{byrne2010}。
	...
	此外，在每棵树的构建过程中，随机森林还会在每个节点上随机选择特征进行分裂，这进一步减少了树之间的相关性，有助于降低过拟合风险 \cite{breiman_random_2001}。
	该算法通过随机选取训练数据的子集和特征来生成每棵树，从而降低各棵树之间的相关性，提高模型的鲁棒性和准确性 \cite{liaw2002random}。
	随机森林具有良好的抗过拟合能力和较高的泛化性能，特别适用于处理高维数据和缺失值 \cite{https://doi.org/10.1890/07-0539.1}。
	...
	在 Python 中，利用 \texttt{scikit-learn} 库 \cite{scikit-learn} 可以方便地实现随机森林模型，用户只需指定树的数量和其他参数，即可训练和评估模型。
	总之，随机森林凭借其高效的性能和广泛的应用场景，成为了机器学习领域的重要工具之一。
	随机森林的优点在于其抗过拟合能力和高准确性。通过结合多个决策树的预测结果，随机森林通常能够提供更为稳定和准确的预测。同时，它还可以评估特征的重要性，使得特征选择过程更加直观。然而，随机森林也存在一些缺点，模型复杂性较高，训练和预测的时间成本相对较大，并且相比单棵决策树，其可解释性较差，难以直观理解模型的决策逻辑。
	Feng Q（2015）利用无人机获取光学影像，并结合灰度共生矩阵提取纹理特征，通过随机森林分类器提取中国余姚市的城市内涝淹没区域，结果表明该方法在城市洪水监测中表现良好，分类精度高于最大似然法和人工神经网络。\cite{prasad_newer_2006}
	Prasad, A.M.（2006）评估了回归树分析（RTA）、Bagging Trees（BT）、随机森林（RF）和多元自适应回归样条（MARS）四种统计模型在预测当前及未来气候情景下植被分布的效果，使用加拿大气候中心的全球环流模型对美国东部四种树种进行了测试，结果表明BT和RF在再现当前分布及预测未来适生地方面表现最佳，RTA在某些树种上具有可解释性，而MARS在未来气候预测中效果较差。\cite{w7041437}
	Ewunetu（2025）使用谷歌地球引擎（GEE）和随机森林（RF）分类器，结合多年的Landsat遥感数据和当地居民与长老的实地数据，分析了上特克兹盆地的土地利用/土地覆盖变化及其原因\cite{ewunetu_integrating_2025}。此外，Eisavi, V.等（2015）使用Landsat 8多时相光谱和热成像数据的潜力，采用随机森林分类器对伊朗西阿塞拜疆省Naghadeh市及其周边地区进行土地覆盖分类，结果表明，选取重要特征和多时相热信息能显著提高分类精度\cite{eisavi_land_2015}。
	...
	XGBoost是一种基于决策树的机器学习算法，因其速度和性能在处理大规模数据和复杂问题时非常受欢迎 \cite{10.1145/2939672.2939785}。
	XGBoost通过优化损失函数和增加正则化项，减少过拟合风险，同时加速模型的训练过程。其关键特性之一是实现了并行计算，使得在处理大规模数据集时具有显著的速度优势。此外，XGBoost 还提供了多种灵活的参数配置，使得用户能够针对具体问题进行调优。该模型不仅适用于分类和回归任务，还在许多机器学习竞赛中表现出色。由于其强大的性能和灵活性，XGBoost 广泛应用于金融、医疗、广告和推荐系统等多个领域，是现代机器学习中不可或缺的重要工具。然而，XGBoost 也有一些缺点。首先，它相较于其他简单的模型如线性回归，需要更多的时间和资源来训练，尤其是当数据量非常大时。其次，XGBoost的超参数调优较为复杂，错误的设置可能导致模型表现不佳。此外，尽管 XGBoost 的强大性能在大多数情况下表现出色，但它的解释性较差，不如简单模型容易解读。
	对于某些问题，XGBoost 的复杂性可能带来过拟合风险，尤其是当训练数据的规模和质量不足时。
	总体来说，XGBoost 非常适合高维度数据集和需要高精度的应用场景，但在某些情况下可能需要平衡其复杂性与可解释性。
	Xu Guo（2023）通过引入降水、蒸发和地面温度等气候因素，并考虑了23个地下水条件因素，采用随机森林、XGBoost和LightGBM三种集成学习模型，探讨了银川平原南部地区的地下水潜力绘制，并发现考虑气候因素的模型表现优于不考虑的模型。\cite{GUO2023129599}Wenkai Yan（2022）提出了一种基于XGBoost算法的新模型，将气象因素输入用于森林火灾风险评级预测，并通过2016至2017年重庆省的数据验证，模型精度为55.10\%，优于传统方法如逻辑回归和支持向量机\cite{9700827}。在环境变化监测方面，Beichen Zhang提出了一种基于极端梯度提升（XGBoost）模型的框架，用于预测德克萨斯州的多类别干旱影响，并通过标准化降水指数（SPI）将其与干旱影响报告中的文本数据相连接，研究结果表明该模型在评估干旱对农业、火灾、社会健康、植物和野生动物等方面的影响方面表现出色，并为美国干旱风险管理提供了可能的评估方法。\cite{zhang2020quantitative}
	在生态灾害监测中，Shiqi Zhou（2022）利用XGBoost、SHAP和PDP算法构建级联模型链，探讨了建筑配置对城市暴雨洪涝风险的影响，并为城市规划者在建筑指标视角下制定应对城市暴雨洪涝风险的策略提供了实践指导\cite{ZHOU2022104235}。
	
	\subsection{LightGBM}
	
	LightGBM（Light Gradient Boosting Machine）是一种高效的梯度提升框架，专为处理大规模数据集和高维特征而设计 \cite{NIPS2017_6449f44a}。
	...
	在城市土地利用研究中，C. Candido（2021）提出了一种基于时间序列Sentinel-2卫星影像的土地覆盖分类方法，使用了Light Gradient Boosting Machine（LightGBM）算法进行不同土地覆盖类型的分类\cite{CANDIDO2021100565}。
	在环境监测领域，Banik, R（2024）提出了一种基于季节性自回归积分滑动平均模型（SARIMA）和LightGBM组合的集成方法，用于提高特里普拉地区稻作栽培的降水预测准确性，并通过与其他现有模型的比较，验证了该集成模型在长期降水预测中的有效性和可行性。\cite{banik_rainfall_2024}。
	Sun Y（2022）将 LightGBM应用于森林火灾易发性预测，并与随机森林（RF）和逻辑回归（LR）模型进行了比较分析，验证了LightGBM在该领域的有效性。\cite{rs14174362}。
	
	\subsection{分位数随机森林}
	
	分位数回归（Quantile Regression）是一种用于估计不确定性的传统方法 \cite{10.1257/jep.15.4.143}，其输出能够反映未观测数据点的不确定性范围（上界和下界）。
	...
	其中，\( \alpha \) 是介于 0 和 1 之间的分位数值， \( \alpha = 0.05 \) 对应 5\% 分位数（下界），\( \alpha = 0.95 \) 对应 95\% 分位数（上界）。通过估计这些分位数，QRF 能够提供比传统均值回归更丰富的不确定性信息，在气候预测、金融风险管理等领域具有广泛应用。
	...
	SHAP 图利用了 Shapley 值的加性特性，将各个特征的贡献加和成模型的预测值，以可视化模型输出对输入特征的敏感性和依赖性 \cite{NIPS2017_8a20a862}。	在计算单个特征的 SHAP 值时，所有可能的特征组合都被考虑，并通过求取平均边际贡献来表示该特征的贡献大小。
	...
	适合开发各种规模和复杂度的机器学习模型 \cite{abadi2016tensorflowlargescalemachinelearning}。
	...
	在遥感影像分析方面，L. Mou（2018）提出了一种新的网络架构—递归卷积神经网络（ReCNN），旨在学习多光谱图像中的光谱-空间-时间特征表示，用于变化检测。\cite{8517375}在气候变化研究中，Sun L（2021）利用卷积神经网络（CNN）对中国的每日气温和降水进行统计降尺度，比较不同模型配置的表现，并与传统方法（如广义线性回归模型和偏差校正方法）进行对比，发现CNN在再现地理分布、季节变化和极端百分位方面具有优势，但在时间相关性上仍逊于再分析数据。\cite{https://doi.org/10.1002/joc.6769}在城市土地利用变化研究中，Cheng W(2023)提出了一种名为UrbanGenoGAN的新算法，结合生成对抗网络（GAN）、遗传优化算法（GOA）和地理信息系统（GIS），用于优化城市规划，并通过比较分析和案例研究验证其在提高规划效果、可扩展性和计算效率方面的优势。\cite{cheng2023urbangeno}在生态监测领域，Pang Y.（2022）利用人工智能算法（人工神经网络、径向基函数网络、支持向量机和随机森林）融合多源数据（火点、气象、地形、植被和社会经济数据）来预测中国各省的森林火灾发生概率，并绘制了森林火灾高风险区域分布图。\cite{rs14215546}在海洋环境监测和物种识别方面，Hsiung AR(2020)讨论了开发一种自动识别鱼类物种的系统，利用图像处理和深度学习技术，旨在提高公众意识并通过减少人工观察和分析中的错误来改善渔业教育\cite{hsiung_ar_little_2020}。总的来说，TensorFlow是一个功能强大且适用于各种机器学习任务的框架，特别适合大规模分布式训练和深度学习模型的开发，但在易用性和调试方面需要较高的技术门槛。
	...
	\section{贝叶斯回归模型}
	通过加载必要的库 \texttt{bayesplot} \cite{bayesplot}、\texttt{rstanarm} \cite{rstanarm} 和 \texttt{ggplot2} \cite{Villanueva03072019}，我们设置工作目录并读取数据文件。
	...
	计算完成后，整理并格式化了一个包含这些结果的数据框，最后使用 \texttt{xtable} 包将该数据框转化为 LaTeX 格式的表格。
...
%	\end{figure}
	...
	我们加载了几个常用的 R 包，包括 caret\cite{2015ascl.soft05003K}、ggplot2、dplyr\cite{wickham2015dplyr}、tidyr\cite{tidyr} 和 ggthemes\cite{ggthemes}，分别用于机器学习、绘图、数据处理和整理。
...
%	
	...
	随后，我们通过特定规则对数据的列名进行了标准化处理，去除无关后缀以及重复部分。
	...
	在绘制图形时，我们使用了\texttt{matplotlib}\cite{4160265}创建了一个 2x2 的子图布局，
...
%	\end{figure}	
	...
	\begin{table}[H]
		...
		\caption*{Training and Validation Loss Curves of Neural Network Regression Model: MSE and R Square}
...
%	\end{figure}
	...
	\section{密集神经网络模型和扩张卷积神经网络}
	密集神经网络模型（DenseNet）是一个包含多个密集块（DenseBlock）和过渡层（TransitionLayer）的网络架构。密集块使用了瓶颈层（BottleneckLayer）来提取特征，每个瓶颈层由两个卷积层组成，用以捕捉数据的不同层次信息。过渡层则用于降低特征图的尺寸，同时保持有用的特征信息。每个密集块和过渡层的组合帮助模型在处理数据时逐步提取更高级的特征。
	
	同时我们实现了一个基于扩张卷积（WaveNet）架构的回归模型，用于预测病原负荷（Pathogen Load）。WaveNet架构，这是一种基于扩张卷积（dilated convolution）的神经网络架构，旨在捕捉时间序列中的长期依赖关系。模型将特征经过多个卷积层后，通过Dropout层来防止过拟合，最终输出一个表示病原负荷的数值。在卷积层的设计中，采用了不同的扩张率来捕获不同尺度的时间特征，从而增强了模型对复杂模式的学习能力。在模型编译和训练过程中，采用了均方误差（MSE）作为损失函数，并使用Adam优化器进行优化。为了避免过拟合，训练过程中使用了早停策略（EarlyStopping），当验证集上的损失不再改善时，提前停止训练。训练过程中，模型的最佳权重会被保存下来。在第一个子图中，我们将各个模型的训练损失曲线绘制出来，横坐标是训练的 epoch 数，纵坐标是损失值；第二个子图绘制了每个模型的验证损失曲线，展示了验证阶段的损失变化。两个子图的 Y 轴范围都设置为 10 到 50，以便于比较不同模型的损失变化趋势，并且为每个曲线添加了图例，方便区分不同模型。接下来，我们使用另外一组文件（包含每个模型的实际值与预测值对比数据）来绘制实际值与预测值之间的关系。首先，denseNet 在最优训练集和验证集损失上表现较好，最优训练集损失为 41.26，而最优验证集损失则为 35.57，这表明该模型能够较好地拟合数据，并在验证集上保持较低的误差。相比之下，wavenet1 的最优训练集损失为 771.14，验证集损失为 145.74，显示出该模型在训练过程中存在较大波动，未能达到较好的拟合效果。对于 wavenet2，其最优训练集损失为 21.29，最优验证集损失为 21.38，二者接近，且验证集损失为所有模型中最低，表明该模型在训练集和验证集上的表现都较为优秀，可能存在较为平衡的拟合情况。最后，wavenet5 的最优训练集损失为 16.30，最优验证集损失为 22.81，虽然训练集损失较低，但验证集损失相对较高，可能表明该模型在训练集上过拟合，无法很好地推广到验证集。综合来看，wavenet2 在所有模型中表现最佳，其验证集损失最低，而 wavenet1 的表现相对较差。
	
	
\chapter{讨论}
本研究基于多源大尺度环境数据，结合机器学习和深度学习方法，成功构建了一个植物病害预测模型，并揭示了气候和土壤因子对植物病害的驱动机制。研究结果表明，环境因素对植物病害的影响复杂且多变，体现出空间和时间的显著变化特征，提供了新的视角来理解植物病害发生的规律及其预测。通过贝叶斯统计模型分析，发现土壤砂含量对植物病害的发生有显著的正向影响。这一发现强调了土壤类型在病害传播中的重要性，尤其是土壤质地可能会影响水分、养分的保持及病原菌的生长环境。此外，经度和海拔对病害的影响呈现负相关，这可能与地理环境的变化导致的气候条件差异有关。经度和海拔可能反映了不同地区气候变化模式以及微气候条件的差异，从而影响病原菌的传播和植物的免疫反应。然而，太阳辐射、风速和气压等因素的影响较为不确定，表明在不同的环境条件下，这些变量的作用可能会有显著差异，这也为未来的研究提供了进一步探索的空间。通过集成学习方法（如随机森林和梯度提升机）的比较，本研究在植物病害预测中得到了较为准确的结果。集成学习方法能够结合多个模型的优势，减少单一模型可能存在的偏差，提高预测的准确性。尤其是在处理复杂的环境数据时，随机森林算法表现出较强的预测能力。通过随机森林模型，研究不仅绘制了中国地区植物病害的大尺度预测图，还模拟了气候变化情境下植物病害的分布。这一成果为农业管理者和决策者提供了在全球气候变化背景下的科学依据，帮助其评估未来病害风险，并制定合理的防控策略。本研究还进一步采用了深度学习方法（如卷积神经网络和全连接神经网络），并对模型进行了训练与优化。深度学习方法能够更好地捕捉数据中的复杂非线性关系，尤其适合处理大规模且高维的环境数据。通过与传统机器学习模型的比较，深度学习模型在某些情况下能够提供更高的预测精度，尤其是在数据集足够大的情况下。然而，深度学习模型通常需要较长的训练时间以及更多的计算资源，这可能限制其在实际应用中的普及。未来的研究可以探讨如何进一步优化这些深度学习模型，使其在预测精度和计算效率之间取得更好的平衡。尽管本研究在植物病害预测领域取得了一定的进展，但仍存在一些局限性。首先，尽管研究涉及了多种气候和土壤变量，但一些可能影响植物病害的因素（如病原菌的种类和传播路径、农作物的种植密度与管理措施等）未能纳入研究。未来的研究应考虑更全面的因素，以提高模型的准确性和可靠性。其次，本研究的数据主要来自中国尽管涉及了全国26个省级行政区，但数据的空间覆盖范围和样本量仍有待进一步扩大，尤其是在不同类型的农田和气候区域的代表性。最后，气候变化对植物病害的影响尚未得到充分研究，气候变化情景下植物病害的动态预测仍需进一步验证和优化。本研究为植物病害的预测提供了新的思路，但仍有许多研究方向值得进一步探讨。未来的研究可以扩展数据源，整合更为丰富的环境和作物管理信息，以提高预测模型的精度。总的来说，本研究通过机器学习与深度学习技术，探索了植物病害预测的新方法，为农业病害管理和全球气候变化背景下的病害防控提供了科学依据和技术支持。然而，要实现更加精准和高效的病害预测，还需要进一步完善模型，整合更多的数据源，并加强对气候变化和环境因子的长期监测与分析。
\chapter{总结与展望}

本研究通过整合多源大尺度环境数据（包括气候和土壤数据），并采用机器学习和深度学习方法，成功构建了一个植物病害预测模型。通过对中国的实地调查数据进行分析，揭示了不同环境变量（如土壤砂含量、气候因子、地理位置等）对植物病害的影响。贝叶斯统计分析结果表明，土壤砂含量对植物病害的发生具有显著的正向影响，而经度和海拔则呈现负相关关系。集成学习方法（如随机森林和梯度提升机）进一步证明了这些环境因素在植物病害预测中的重要性，并为中国地区提供了大尺度的植物病害预测图。
通过深度学习模型（如卷积神经网络和全连接神经网络）的训练与优化，本研究进一步提高了预测精度。深度学习模型的比较与优化，显示了神经网络在植物病害预测中的潜力，尤其是在处理复杂和大规模数据集时。虽然本研究为植物病害的预测提供了有价值的思路和方法，但仍存在一些局限性和进一步研究的空间。首先，尽管本研究采用了多种机器学习和深度学习方法，但其模型仍可能受到数据质量和样本数量的限制，未来可以通过扩大数据集规模，进一步提升模型的泛化能力。其次，气候变化对植物病害的影响仍需进一步深入探讨，尤其是在不同气候情景下，植物病害的发生模式和预测模型的适应性。未来的研究可以通过更多的长期气候数据和多区域的研究，验证和优化模型的适应性。
环境变量的时空动态变化可能对病害发生产生更复杂的影响，未来可以进一步探索时序分析方法，如长短时记忆网络（LSTM），以提高对实时监测环境变化并准确预测病害的能力（	K. S. Patle，2022）\cite{Patle2022}。
综上所述，本研究不仅为植物病害的预测提供了新的思路，也为应对全球气候变化背景下的农业可持续发展提供了重要的理论依据和技术支持。未来的研究将继续深化气候变化对植物病害的影响机制，推动智能农业和精准农业的发展。
